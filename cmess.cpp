...
Expr *CMessRecursiveASTVisitor::VisitBinaryOperator(BinaryOperator *BO)
{
	...
	//	 llvm::outs() << rewrite.ConvertToString(cast<Stmt>(BO)) << "\n";
	 }
	return BO;
}
...
/*class CMessFrontendAction : public clang::ASTFrontendAction {
	public:
	  virtual clang::ASTConsumer *CreateASTConsumer(CompilerInstance &Compiler, llvm::StringRef InFile) {
		   return new CMessASTConsumer(&rewrite);
	  }
};*/
/*
...
/*
	...
	Tool.run(newFrontendActionFactory<CMessFrontendAction>());
	return 0;
*/
	struct stat sb;
	if(argc < 2){
